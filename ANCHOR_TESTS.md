...
### Intel page slim — Campaigns / Chains / MITRE sub-tabs deleted (2026-05-15 PR-B)

Operator simplification: Intel keeps only the Profiles list + Baseline (Baseline moves to Health in PR-C). The Campaigns sub-tab becomes a Cases-header tag in PR-D. The Chains aggregate timeline goes away because per-incident chains already render on the Cases journey panel. The MITRE coverage heatmap goes away because the Monthly threat report already carries it. PR-B drops the three sub-tab buttons, the three loaders (`loadCampaigns`, `loadChains`, `loadMitreCoverage`), and the corresponding branches from `switchIntelTab`. No dead code.

- `crates/agent/src/dashboard/mod.rs::tests::pr_intel_slim_deleted_sub_tab_buttons_are_gone_from_index_html` — `id="intelTabCampaigns"` / `intelTabChains` / `intelTabMitre` and their `switchIntelTab('campaigns'|'chains'|'mitre')` onclick handlers stay deleted; visible button labels `>Campaigns</button>` / `>Chains</button>` / `>MITRE</button>` are absent. Profiles + Baseline buttons remain present.
- `crates/agent/src/dashboard/mod.rs::tests::pr_intel_slim_deleted_sub_tab_loaders_are_gone_from_intel_js` — `loadCampaigns`, `loadChains`, `loadMitreCoverage` declarations AND call sites all stay deleted in intel.js (no orphan loaders, no orphan dispatch).
- `crates/agent/src/dashboard/mod.rs::tests::pr_intel_slim_switch_tab_dispatcher_lists_only_remaining_tabs` — `switchIntelTab` body references none of `'Campaigns'`/`'Chains'`/`'Mitre'` and their lowercase variants; the `tabs` array reads `['Profiles', 'Baseline']`; the dispatcher routes `'baseline' → loadBaseline()` with a default-fallback to `loadIntel()` (Profiles).
